...
export const Kasir = () => {
  ...
  const cari = (e) => {
    ...
    const list = produk.filter((item) =>
      item.namaProduk.toLowerCase().includes(cariRef.current.value)
    );
    console.log(list);
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      await axios
        .get("https://api.drnich.co.id/api/pos/kasir/transaksiinvoice")
  ...
  const pilihJenis = () => {
    if (jenisRef.current.value == "reset") {
      ...
      const filter = kategori.filter(
        (item) => item.jenis._id == jenisRef.current.value
      );
  ...
  const pilihKategori = () => {
    if (kategoriRef.current.value == "reset") {
      ...
      const filter = produk.filter(
        (item) => item.kategori._id == kategoriRef.current.value
      );
  ...
  useEffect(() => {
    ...
    cart.map((item) =>
      setTotal(
        (prev) => prev + item.jumlah * item.hargaJual,
    ...
    const kalkulasi = async () => {
      await axios
        .post("https://api.drnich.co.id/api/pos/kasir/kalkulasiharga/", datax)
        .then((response) => {
      ...
      <div className="flex flex-col px-5 py-8 gap-1 bg-white w-full h-fit min-h-full pt-8 text-[#454545] text-[12px]">
        ...
        <div className="w-full grid place-items-start h-fit border-b border-t my-2">
          ...
          {cart.length > 0 ? (
            cart.map((item, i) => (
              <button
                key={i}
                className="grid w-full place-items-start gap-1 border border-[#BDBDBD] rounded-xl p-4 mt-4 "
              >
            ...
            <p className="w-full my-3 text-center text-[#BDBDBD]">
              Belum ada data
            </p>
          )}
        </div>
        ...
        {produkTampil.map((item, i) => (
          <button
            ...
            key={i}
            className={`grid w-full place-items-start gap-1 rounded-xl p-4 mt-4 ${
              cart.some((itemx) => itemx._id == item._id)
                ? " border border-[#C2A353]"
