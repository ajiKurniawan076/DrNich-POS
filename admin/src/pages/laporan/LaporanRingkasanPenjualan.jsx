...
export const LaporanRingkasanPenjualan = () => {
  ...
  useEffect(() => {
    const fetchChart = async () => {
      try {
        const tanggal = {
          tanggal: new Date().toISOString().split(".")[0] + "Z",
          menu: atur,
        };
        const response = await axios.post(
          "https://api.drnich.co.id/api/pos/laporan/laporangrafik",
          tanggal
        );
        ...
        <div>
          ...
          {chartData.length > 0 && (
            ...
            >
              ...
              >
                {" "}
                {/* Ensures BarChart does not shrink */}
          ...
          <div className="grid text-[12px] text-[#454545] gap-[10px]">
            ...
            {topPromo.map((item, index) => (
              <div
                key={index}
                className="flex justify-between text-center items-center border border-[#BDBDBD] rounded-xl p-[15px]"
              >
          ...
          <div className="flex flex-col gap-[10px] h-fit">
            <div className="flex flex-col gap-[10px] mt-[30px] h-fit">
              {topCustomers.map((customer, index) => (
                <div
                  key={index}
                  className="flex justify-between p-[15px] border border-[#BDBDBD] rounded-xl text-[12px]"
                >
