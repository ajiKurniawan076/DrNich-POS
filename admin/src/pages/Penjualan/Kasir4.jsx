...
return (
    <modaltransaksi.Provider value={{modalPel, setModalPel, pelanggan, pelangganTerpilih, setPelangganTerpilih, modalPro, promoTerpilih, setPromoTerpilih, setModalPro, promo}}>
    <div className={`fixed z-50 top-0 start-0 w-full overflow-auto inset-0 ${modal ? '' : 'hidden'}`}>
    ...
    <button 
        ...
        {cart.map((item, i)=>(
            ...
            <p className='font-semibold'>Rp {item.hargaJual*item.jumlah}</p>
        </div>
        </>
        ))}
        ...
        className='flex justify-center items-center text-center border rounded-xl bg-[#DCDCDC] font-bold text-[14px] text-white p-4 w-full'>Batal</button>
    </div>
    </div>
    <PilihPelanggan/>
    <PilihPromo/>
